...
void loop() {
  if (IrReceiver.decode()) {
    ...
    // Map your remote codes here:
    // Button 1 -> Red
    // Button 2 -> Green
    // Button 3 -> Blue
    if (IrReceiver.decodedIRData.command == 0x0C) { // Example: Button 1
      redState = !redState;
      digitalWrite(LED_RED, redState ? HIGH : LOW);
    } else if (IrReceiver.decodedIRData.command == 0x18) { // Example: Button 2
      greenState = !greenState;
      digitalWrite(LED_GREEN, greenState ? HIGH : LOW);
    } else if (IrReceiver.decodedIRData.command == 0x5E) { // Example: Button 3
      blueState = !blueState;
  ...
  try {
    ...
    if (count === 0 || force) {
      if (force) {
        ...
        console.log('Cleared existing projects for reset.');
      }
      await Project.insertMany(INITIAL_PROJECTS);
      console.log(`✅ Seeded ${INITIAL_PROJECTS.length} default projects with YouTube demonstration link.`);
    } else {
      console.log(`Database already has ${count} projects. Skipping auto-seed.`);
    }
  } catch (err) {
